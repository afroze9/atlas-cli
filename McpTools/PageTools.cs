...
{
    [McpServerTool(Name = "confluence_page_list"), Description("List Confluence pages, optionally filtered by space, title, or status")]
        ...
        [Description("Filter by status: current, draft, trashed")] string? status = null)
    {
        try
        {
            var result = await PageService.ListAsync(spaceId, title, limit, status);
    ...
    [McpServerTool(Name = "confluence_page_create"), Description("Create a new Confluence page")]
        ...
        [Description("Page status: current or draft (default: current)")] string status = "current")
    {
        try
        {
            var result = await PageService.CreateAsync(spaceId, title, body, bodyFormat, parentId, status);
            return McpAtlasHelper.ToJson(result);
        }
        catch (AtlasApiException ex) { return McpAtlasHelper.HandleApiError(ex); }
        catch (Exception ex) { return McpAtlasHelper.HandleException(ex); }
